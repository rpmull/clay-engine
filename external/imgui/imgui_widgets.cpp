...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    const bool use_fixed_row_height = style.TreeNodeRowHeight > 0.0f;
    ...
    const float default_row_height = g.FontSize + style.FramePadding.y * 2;
    const float frame_height = use_fixed_row_height
        ? ImMax(style.TreeNodeRowHeight, label_size.y + padding.y * 2)
        : ImMax(ImMin(window->DC.CurrLineSize.y, default_row_height), label_size.y + padding.y * 2);
    ...
    if (use_fixed_row_height)
        text_pos.y = frame_bb.Min.y + ImMax(padding.y, (frame_height - label_size.y) * 0.5f);
    ...
    // Render
    {
        ...
        const float bullet_center_y = use_fixed_row_height ? (frame_bb.Min.y + frame_height * 0.5f) : (text_pos.y + g.FontSize * 0.5f);
        const float arrow_top_y_framed = use_fixed_row_height ? (frame_bb.Min.y + (frame_height - g.FontSize) * 0.5f) : text_pos.y;
        const float arrow_top_y_unframed = use_fixed_row_height ? (frame_bb.Min.y + (frame_height - g.FontSize) * 0.5f) : (text_pos.y + g.FontSize * 0.15f);
        if (display_frame)
        {
            // Framed type
            ...
            if (flags & ImGuiTreeNodeFlags_Bullet)
                RenderBullet(window->DrawList, ImVec2(text_pos.x - text_offset_x * 0.60f, bullet_center_y), text_col);
            else if (!is_leaf)
                RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, arrow_top_y_framed), text_col, is_open ? ((flags & ImGuiTreeNodeFlags_UpsideDownArrow) ? ImGuiDir_Up : ImGuiDir_Down) : ImGuiDir_Right, 1.0f);
        ...
        else
        {
            ...
            if (span_all_columns && !span_all_columns_label)
                TablePopBackgroundChannel();
            if (flags & ImGuiTreeNodeFlags_Bullet)
                RenderBullet(window->DrawList, ImVec2(text_pos.x - text_offset_x * 0.5f, bullet_center_y), text_col);
            else if (!is_leaf)
                RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, arrow_top_y_unframed), text_col, is_open ? ((flags & ImGuiTreeNodeFlags_UpsideDownArrow) ? ImGuiDir_Up : ImGuiDir_Down) : ImGuiDir_Right, 0.70f);
            if (g.LogEnabled)
                LogSetNextTextDecoration(">", NULL);
        }
