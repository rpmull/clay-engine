...
struct IMGUI_API ImGuiDockNode
{
    ...
    ImGuiID                 TabBarInitSelectedTabId;    // [Leaf node only] Preferred tab when tab bar is created (builder override).
    bool                    TabBarInitRequest;          // [Leaf node only] Apply TabBarInitSelectedTabId on next update.
    int                     TabBarInitRetryRemaining;   // Countdown allowing deferring the request for a few frames.
...
IMGUI_API void          DockBuilderSetNodeActiveWindow(ImGuiID node_id, const char* window_name);
