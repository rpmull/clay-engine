...
uniform vec4 u_PBRScalar0;
uniform vec4 u_PBRScalar1;
uniform vec4 u_EmissionColor;
uniform vec4 u_TextureUsage;
...
// PBR lighting uses shared lib_pbr_common.sh
...
void main()
{
    ...
    if (u_TextureUsage.z > 0.5) ao *= texture2D(s_ao, uv).r;
    ao = clamp(ao, 0.0, 1.0);
    ...
    if (u_TextureUsage.w > 0.5) emission *= texture2D(s_emission, uv).rgb;
    finalColor += emission;
